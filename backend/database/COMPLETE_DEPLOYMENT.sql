-- =====================================================================
-- FAREDEAL COMPLETE DATABASE DEPLOYMENT
-- =====================================================================
-- Business: FAREDEAL POS System
-- Country: Uganda
-- Currency: UGX (Ugandan Shillings)
-- Deployment: All Portals (Admin, Manager, Cashier, Supplier)
...
-- =====================================================================
-- SECTION 1: ADMIN PORTAL TABLES
-- =====================================================================

-- 1.1 Admin Activity Log
...
-- 1.4 Admin System Configuration
...
-- 1.5 Admin Employee Access Control
...
CREATE INDEX idx_admin_notifications_target ON admin_notifications(target_admin_id);
CREATE INDEX idx_admin_notifications_is_read ON admin_notifications(is_read);
CREATE INDEX idx_admin_employee_access_employee ON admin_employee_access(employee_id);
CREATE INDEX idx_admin_payment_audit_created_at ON admin_payment_audit(created_at);
CREATE INDEX idx_admin_error_logs_created_at ON admin_error_logs(created_at);
CREATE INDEX idx_admin_error_logs_error_level ON admin_error_logs(error_level);

-- =====================================================================
-- SECTION 2: MANAGER PORTAL TABLES
-- =====================================================================

-- 2.1 Manager Activity Log
...
CREATE TABLE IF NOT EXISTS manager_daily_reports (
    ...
    team_attendance_rate DECIMAL(5,2) DEFAULT 0.00,
...
-- 2.5 Manager Employee Schedules
...
-- 2.6 Manager Employee Attendance
...
-- 2.9 Manager Customer Complaints
...
-- 2.11 Manager Performance Metrics
...
-- Manager Indexes
CREATE INDEX idx_manager_activity_manager_id ON manager_activity_log(manager_id);
CREATE INDEX idx_manager_daily_reports_manager_id ON manager_daily_reports(manager_id);
CREATE INDEX idx_manager_daily_reports_date ON manager_daily_reports(report_date);
CREATE INDEX idx_manager_teams_manager_id ON manager_teams(manager_id);
CREATE INDEX idx_manager_schedules_manager_id ON manager_employee_schedules(manager_id);
CREATE INDEX idx_manager_schedules_date ON manager_employee_schedules(schedule_date);
CREATE INDEX idx_manager_attendance_manager_id ON manager_employee_attendance(manager_id);
CREATE INDEX idx_manager_stock_requests_manager_id ON manager_stock_requests(manager_id);
CREATE INDEX idx_manager_alerts_manager_id ON manager_alerts(manager_id);
CREATE INDEX idx_manager_alerts_is_read ON manager_alerts(is_read);

-- =====================================================================
-- SECTION 3: CASHIER PORTAL TABLES
-- =====================================================================

-- 3.1 Cashier Profiles
...
CREATE INDEX idx_cashier_drawer_ops_cashier_id ON cashier_drawer_operations(cashier_id);
...
CREATE INDEX idx_cashier_activity_cashier_id ON cashier_activity_log(cashier_id);

-- =====================================================================
-- SECTION 4: SUPPLIER PORTAL TABLES
-- =====================================================================
...
CREATE INDEX idx_supplier_activity_supplier_id ON supplier_activity_log(supplier_id);
...
SELECT 
    ...
    COUNT(DISTINCT mtm.employee_id) AS total_team_members,
    (SELECT COUNT(*) FROM manager_daily_reports WHERE manager_id = m.id AND report_date = CURRENT_DATE) AS report_submitted_today,
    (SELECT COUNT(*) FROM manager_stock_requests WHERE manager_id = m.id AND status = 'pending') AS pending_stock_requests,
    (SELECT COUNT(*) FROM manager_customer_complaints WHERE manager_id = m.id AND status IN ('open', 'investigating')) AS open_complaints,
    (SELECT COUNT(*) FROM manager_alerts WHERE manager_id = m.id AND is_read = false) AS unread_alerts
FROM users m
LEFT JOIN manager_teams mt ON m.id = mt.manager_id AND mt.is_active = true
LEFT JOIN manager_team_members mtm ON mt.id = mtm.team_id AND mtm.is_active = true
...
CREATE POLICY admin_view_all ON admin_activity_log
    FOR SELECT USING (
        EXISTS (SELECT 1 FROM users WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
    );

-- Manager policies (managers can see their own data)
CREATE POLICY manager_view_own_data ON manager_activity_log
    FOR SELECT USING (manager_id = auth.uid());

-- Cashier policies (cashiers can see their own data)
...
CREATE POLICY cashier_view_own_transactions ON cashier_transactions
    FOR SELECT USING (
        cashier_id IN (SELECT id FROM cashier_profiles WHERE user_id = auth.uid())
    );

-- Supplier policies (procurement staff can view)
...
-- =====================================================================
-- SECTION 7: SAMPLE DATA (Uganda specific)
-- =====================================================================
...
-- DEPLOYMENT COMPLETE
-- =====================================================================
-- All tables, indexes, views, and policies have been created successfully!
-- 
-- Next Steps:
-- 1. Verify tables were created: Check "Table Editor" in Supabase
-- 2. Review RLS policies: Check "Authentication > Policies"
-- 3. Test the views: Run SELECT queries on the dashboard views
-- 4. Start your backend server to test API endpoints
-- 
-- Total Tables Created: 43
-- Total Indexes Created: 50+
-- Total Views Created: 3
-- Total RLS Policies: 6
-- =====================================================================

SELECT 'DEPLOYMENT SUCCESSFUL! All portal tables have been created.' AS status;
